...
void AAgentSpawner::Tick(float DeltaTime)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Some warning message")); //TODO zmien hp na inne niz 0
	...
	for (int i = 0; i < Rabbits.Num(); i++) {
		...
		if (rabbit->hp <= RABBIT_MAX_HUNGRY_HP_LEVEL) {
			if (Plants.Num() > 0) {
				...
				rabbit->GetActorLocation() += atractor / atractortDist * RABBIT_VELOCITY;
	...
	for (int i = 0; i < Wolfes.Num(); i++) {
		...
		if (wolf->hp <= WOLF_MAX_HUNGRY_HP_LEVEL) {
			if (Rabbits.Num() > 0) {
				...
				wolf->GetActorLocation() += atractor / atractortDist * WOLF_VELOCITY;
